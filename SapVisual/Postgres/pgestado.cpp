...
bool PgEstado::Borrar(Estado valor)
{
    QSqlQuery query;

    bool flag=query.exec("DELETE FROM estado WHERE codigo='"+valor.getCodigo()+"'");
...
bool PgEstado::Insertar(Estado valor)
{

    QSqlQuery query;
      query.prepare("INSERT INTO estado(nombre, codigo_imagen)"
            " VALUES (?, ?);");
...
bool PgEstado::Actualizar(Estado Antiguo, Estado Nuevo)
{
    QSqlQuery query;
...
Estado PgEstado::Buscar(Estado valor)
{
    QString consulta;

        consulta="SELECT codigo, nombre, codigo_imagen, ruta_img "
                "FROM vista_detalle_estado WHERE ";
...
QMap<QString, ObjetoMaestro *> *PgEstado::BuscarMapa(ObjetoMaestro *valor, QString Extra, CONSULTA tipo)
{
    MapaRepisaGlobal=new QList<ObjetoMaestro*>();

    Estado* val=(Estado*)(valor);
    QString consulta;
...
qint64 PgEstado::Contar()
{
    QString consulta="SELECT count(*) FROM estado";
    qint64 num=0;
...
qint64 PgEstado::ContarConsulta(ObjetoMaestro *valor)
{
    Estado* val=(Estado*)(valor);
    QString consulta;
...
QSqlQueryModel *PgEstado::BuscarTabla(Estado valor, QString Extra, CONSULTA tipo)
{
    QString consulta;
